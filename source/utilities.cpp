...
std::vector<T> make_data_1d(int rows) {
	...
	for (size_t idx = 0; idx < rows; ++idx) {
		data[idx] = static_cast<T>(rows + cosf(idx / kPi) - sinf(cosf(idx)) + tanf(idx) + 2) / rows;
...
//template void print_data_1d(std::vector<int> data);
//template void print_data_1d(std::vector<float> data);
template std::vector<int> make_data_1d(int rows);
template std::vector<float> make_data_1d(int rows);
